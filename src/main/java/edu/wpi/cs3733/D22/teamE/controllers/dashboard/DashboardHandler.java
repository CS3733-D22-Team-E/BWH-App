...
public abstract class DashboardHandler {
    ...
    public DashboardHandler(MedicalEquipmentDAOImpl medicalEquipmentDAO, DashboardController dashboardController) {
        this.medicalEquipmentDAO = medicalEquipmentDAO;
        this.dashboardController = dashboardController;
    }

    protected ArrayList<MedicalEquipment> getEquipmentOnFloor(String floor, ArrayList<MedicalEquipment> allEquipment) throws SQLException {
        String floorID;
        switch (floor) {
            case "LL2":
                floorID = "L2";
    ...
    protected ArrayList<MedicalEquipment> filterByEquipmentType(String equipmentType, ArrayList<MedicalEquipment> allEquipment) {
        ArrayList<MedicalEquipment> filteredByEquipment = new ArrayList<>();
        for (MedicalEquipment curEquipment : allEquipment) {
            if (equipmentType.equals("")
                    || curEquipment.getEquipmentType().equals(equipmentType)) {
    ...
    protected ArrayList<MedicalEquipment> populateEquipmentList(Toggle filter, String floor, String equipmentType, ArrayList<MedicalEquipment> allEquipment) throws SQLException {
        ArrayList<MedicalEquipment> equipmentOnFloor = getEquipmentOnFloor(floor, allEquipment);
        ArrayList<MedicalEquipment> filteredEquipmentByType = filterByEquipmentType(equipmentType, equipmentOnFloor);
        ArrayList<MedicalEquipment> filteredByState = filterEquipment(filteredEquipmentByType, dashboardController.currentFilter);
