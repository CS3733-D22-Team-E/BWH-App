...
public class SanitationRequestDAOImpl implements DAO<sanitationRequest> {
    ...
    public SanitationRequestDAOImpl() throws SQLException {
        ...
        while(rs.next()) {
            ...
            sanitationRequest request = new sanitationRequest(sanitationReqID, otherNotes, floorID, roomID, isUrgent, status, staffAssignee, requestDate.toLocalDate(), deliveryDate.toLocalDate(), sanitationRequest.Size.valueOf(sizeType), sanitationRequest.Biohazard.valueOf(biohazardType));
    ...
    public void update(sanitationRequest item) {
        ...
        try {
            ...
            System.out.println("Add Sanitation Request failed!"); // TODO: Come up with a better catch block
        }
    }
